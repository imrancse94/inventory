        ...
        <div class="ibox">
            <div class="ibox-head">
                <div class="ibox-title">
                    {{__($cmsInfo['subTitle'])}} <a href="{{route('usergroups.index')}}" class="ml-3 btn btn-sm btn-primary pull-right"><i class="fa fa-list-ul"></i> {{__('List')}}</a>
                </div>
            </div>
            <div class="ibox-body">
                    <!-- /.box-header -->
                                                ...
                                                @if(!empty($users))
                                                    @foreach($users as $user)
                                                        @if(!(in_array($user->id, $checkeduser)))
                                                            <li class="list-group-item flexbox {{$isEdit ? 'notsel' : ''}}" data="{{$user->id}}">{{$user->name}}</li>
                                                        @endif
                                                    @endforeach
                                                @else
                                        ...
                                        <div class="col p-0 rounded border border-top-0 border-right-0 border-left-0 border-primary" style="margin: 0 15px;">
                                            <ul class="list-group list-group-bordered" id="sulist">
                                                ...
                                                @if(!empty($checkeduser))
                                                    @foreach($users as $user)
                                                        @if(in_array($user->id, $checkeduser))
                                                            <li class="list-group-item flexbox {{$isEdit ? 'seltd' : ''}}" data="{{$user->id}}"><input type="hidden" name="selected_users[]" value="{{$user->id}}" />{{$user->name}}</li>
                                                        @endif
                                                    @endforeach
                                                @endif
                                                @if(empty($users))
