...
trait PermissionUpdateTreait
{
    public function getPermissionList($user_id)
    {
        ...
        if (!empty($permissions)) {
            if ($user_id == $SUPER_SUPER_ADMIN_ID) {
                ...
                $modules = Module::with('submodules', 'submodules.pages')->whereNotIn('id',[1001,1002])->get();
            }
    ...
    private function getPermittedPageRouteName($permissions)
    {
        ...
        foreach ($permissions as $key => $permission) {
            $permittedRouteName[$key] = strtolower(str_replace(" ", "", $permission->submodule_name)) . "." . strtolower($permission->route_name);
        }
